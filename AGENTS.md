...
It reads and writes flow documents. People receive PDF/A, DOCX, ODT, Markdown, or HTML.
Hangul HWP/HWPX/HML are regional codecs on the same IR. There is no GUI, cursor, undo,
spreadsheet, presentation, or hwpctl layer.

## Crate graph

Depend downward only. Codec crates (`docagent-docx`, `docagent-odt`, `docagent-md`,
`docagent-hwp5`, `docagent-hwpx`, `docagent-hwp3`, `docagent-hml`) must not import
each other or anything at `docagent-layout` and below. `docagent-model` has no
format identifier.
...
6. Corpus, vendor PDFs, and oracles stay out of this repository.
...
All layout coordinates are `i32` at 1/7200 inch (historically called HWPUNIT).
Accumulators are `i64`. `f32`/`f64` are forbidden in `docagent-layout`. LineSeg is a
`LayoutHint` oracle for the Hangul scoreboard; layout must not consume it.
