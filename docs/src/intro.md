...
| Regional | HWP 5, HWPX, HWP 3, HML | East-Asian office formats, same IR. HWP 5 `CHAR_SHAPE`, HWPX `hh:charPr`, HML `CHARSHAPE`, and HWP 3 char-shape attr bits map onto `CharStyle` marks. HWPX hyperlinks are `hp:fieldBegin type="HYPERLINK"`. HML hyperlinks are `FIELDBEGIN Type="Hyperlink"` with `STRINGPARAM Name="Command"`. |
