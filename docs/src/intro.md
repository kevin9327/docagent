...
| OpenDocument | ODT | Default interchange with LibreOffice (`text:h` 18pt/14pt, Quote, `Tcode`, `Tmark`, `CodeBlock`, `HorizontalLine`) |
| Web / notes | Markdown, HTML | Agent-native text (`> quote`, `` `code` ``, ` ```fence``` `, `==mark==`, `- [x]`, `---`) and semantic HTML (`<blockquote>`, `<pre>`, `<mark>`, checkbox, `<hr/>`) |
