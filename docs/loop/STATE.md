...
Last cycle: 39 (2026-09-06) — HWPX HYPERLINK fieldBegin on the same IR
...
Shipped this cycle:
- HWPX writer emits `hp:fieldBegin type="HYPERLINK"` with `hp:stringParam name="Command"` for the URI, display text in `hp:t`, then `hp:fieldEnd`.
- Reader applies the Command URL to the following run as `InlineObject::Hyperlink`.
- First-class letter.md fixture unchanged (input/plan/output hashes match cycle 34–38).
...
1. HWP5/HML/HWP3 hyperlinks, Hangul quotes / code / breaks / tasks, or inline images.
