...
use dochwp_capsule::{record, Capsule, Plan, ENGINE_VERSION};
use dochwp_font::FontSet;
use dochwp_html::to_html;
use dochwp_layout::layout_document;
use dochwp_model::{Diagnostic, Document};
use dochwp_paint::paint;
use dochwp_pdf::to_pdfa;
...
impl Engine {
    ...
    fn convert_document(
        ...
        document
            .diagnostics
            .extend(dochwp_font::resolve_for_document(&document, &fonts));
        ...
        for t in &targets {
            match t {
                ...
                ExportTarget::Hwp5 => {
                    let b = dochwp_hwp5::write(&document).map_err(|e| Error::Hwp5(e.to_string()))?;
                    output_chunks.extend_from_slice(&b);
                    out.hwp5 = Some(b);
                }
                ExportTarget::Hwpx => {
                    let b = dochwp_hwpx::write(&document).map_err(|e| Error::Hwpx(e.to_string()))?;
                    output_chunks.extend_from_slice(&b);
                    out.hwpx = Some(b);
                }
                ExportTarget::Hml => {
                    let b = dochwp_hml::write(&document).map_err(|e| Error::Hml(e.to_string()))?;
                    output_chunks.extend_from_slice(&b);
                    out.hml = Some(b);
                }
                ExportTarget::Docx => {
                    let (b, loss) = dochwp_docx::hwp_to_docx_with_loss(&document)
...
pub fn sniff(bytes: &[u8]) -> Option<InputKind> {
    ...
    if dochwp_docx::sniff(bytes) {
        return Some(InputKind::Docx);
    }
    if dochwp_hml::sniff(bytes) {
...
fn parse_kind(bytes: &[u8], kind: InputKind) -> Result<Document, Error> {
    match kind {
        InputKind::Hwp5 => dochwp_hwp5::read(bytes).map_err(|e| Error::Hwp5(e.to_string())),
        InputKind::Hwpx => dochwp_hwpx::read(bytes).map_err(|e| Error::Hwpx(e.to_string())),
        InputKind::Hwp3 => dochwp_hwp3::read(bytes).map_err(|e| Error::Hwp3(e.to_string())),
        InputKind::Hml => dochwp_hml::read(bytes).map_err(|e| Error::Hml(e.to_string())),
        InputKind::Docx => dochwp_docx::read(bytes).map_err(|e| Error::Docx(e.to_string())),
...
pub fn hwp_docx_loss(doc: &Document) -> Vec<Diagnostic> {
    match dochwp_docx::hwp_to_docx_with_loss(doc) {
        Ok((_, d)) => d,
...
mod tests {
    use super::*;
    use dochwp_model::{Block, Paragraph, Section};
    ...
    fn convert_records_three_hashes() {
        let bytes = dochwp_hwp5::write(&sample_doc()).unwrap();
