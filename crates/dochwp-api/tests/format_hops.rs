...
fn one_hundred_twenty_interformat_hops() {
    ...
    for i in 0..120 {
        let src = make_doc(i);
        let hwp5 = dochwp_hwp5::write(&src).expect("hwp5 write");
        let d1 = dochwp_hwp5::read(&hwp5).expect("hwp5 read");
        assert_core(&src, &d1, "ir->hwp5->ir");
        hops += 1;

        let hwpx = dochwp_hwpx::write(&d1).expect("hwpx write");
        let d2 = dochwp_hwpx::read(&hwpx).expect("hwpx read");
        assert_core(&src, &d2, "hwp5->hwpx->ir");
        hops += 1;

        let hml = dochwp_hml::write(&d2).expect("hml write");
        let d3 = dochwp_hml::read(&hml).expect("hml read");
        assert_core(&src, &d3, "hwpx->hml->ir");
        hops += 1;

        let docx = dochwp_docx::write(&d3).expect("docx write");
        let d4 = dochwp_docx::read(&docx).expect("docx read");
        assert_core(&src, &d4, "hml->docx->ir");
        hops += 1;

        let hwp5b = dochwp_hwp5::write(&d4).expect("docx->hwp5 write");
        let d5 = dochwp_hwp5::read(&hwp5b).expect("hwp5 again");
