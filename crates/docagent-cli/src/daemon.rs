//! `docagentd` — localhost REST adapter over Command/Query.

use std::io::{Read, Write};
use std::net::{TcpListener, TcpStream};

use docagent_api::{Command, Engine, ExportTarget, Query};

fn main() -> Result<(), String> {
    let addr = std::env::var("DOCAGENTD_BIND").unwrap_or_else(|_| "127.0.0.1:7090".into());
    let listener = TcpListener::bind(&addr).map_err(|e| e.to_string())?;
    eprintln!("docagentd listening on {addr}");
