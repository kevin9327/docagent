...
| **Hangul** | HWP 5 / HWPX / HWP 3 / HML as codecs, not as the product name. HWP 5 `CHAR_SHAPE`, HWPX `hh:charPr`, HML `CHARSHAPE`, and HWP 3 char-shape attr bits keep bold / italic / underline / super / sub on the same IR. HWP 5 `%hlk` field, HWPX `hp:fieldBegin type="HYPERLINK"`, and HML `FIELDBEGIN Type="Hyperlink"` keep URI + display text. |
